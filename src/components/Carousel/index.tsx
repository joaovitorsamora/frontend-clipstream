...
export const Carousel: React.FC<CarouselProps> = ({ id, title, url, description, site, image, views, onClickLink }) => {
  return (
    <div className="flex flex-col w-full h-full">
      <div
        className="flex flex-col h-[394px] bg-zinc-900 text-white rounded-xl p-4 cursor-pointer w-full"
        onClick={() => onClickLink(url, id)}
      >
        <div className="flex flex-col flex-grow">
          <h3 className="text-lg font-bold mb-1 flex items-center">
            {title} - {site}
            <span className="ml-2 text-sm font-semibold text-zinc-400">{views}</span>
